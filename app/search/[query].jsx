...
const Search = () => {
  ...
  const { data: profile, refetch} = useAppwrite(() => searchProfile(query, user.$id));
        ...
        renderItem={({ item }) => {
          ...
          return(
            <TouchableOpacity
              onPress={() => {
                setOProfile(item.$id);
                const newProfile = item.$id;
                if (pathname.startsWith("/vprofile")) {
                  router.setParams({ oprofile: newProfile });
                } else {
                  router.push(`/vprofile/${newProfile}`);
                }
              }}
